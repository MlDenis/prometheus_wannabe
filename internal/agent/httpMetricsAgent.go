...
func (p *httpMetricsAgent) Send(ctx context.Context, metrics []metrics.Metric) error {
	fmt.Printf("Push %v metrics", len(metrics))
	...
	for _, metric := range metrics {
		...
		if err != nil {
			fmt.Printf("Fail to create push request: %v", err.Error())
			return err
		}
		...
		if err != nil {
			fmt.Printf("Fail to push metric: %v", err.Error())
			return err
		}
		...
		if err != nil {
			fmt.Printf("Fail to read response body: %v", err.Error())
			return err
		}
